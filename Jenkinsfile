...
export ENV=test
export EPOCH=$(date +%s)
export TAG="${ENV}-${app}-${EPOCH}"

oc new-app -f ${template} -p APPLICATION_NAME=${ENV}-${app}-${EPOCH} -p IMG_TAG=${TAG} -p MYSQL_DB_SETUP=YES -p NAMESPACE=${namespace} -p SOURCE_REPOSITORY_URL=${github_repo} -n ${namespace} --dry-run -o yaml | oc apply -f - -n ${namespace}
